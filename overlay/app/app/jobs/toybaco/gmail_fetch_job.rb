...
require_relative '../../../lib/toybaco/connections/gmail_sync'
require_relative '../../../lib/toybaco/connections/gmail_ingest'

class Toybaco::GmailFetchJob < ApplicationJob
  ...
  def with_lease
    ...
    @lease_key = "TOYBACO_GMAIL_FETCH::#{@channel.id}"
  ...
  def fetch
    ...
    catch(:stale_gmail_connection) do
      state = Toybaco::Connections::GmailSync.new(api: integration.api).step(
  ...
  def with_current_connection
    @channel.with_lock do
      throw :stale_gmail_connection unless current_connection? && integration.allowed?(@channel.account)
      throw :stale_gmail_connection unless Redis::Alfred.get(@lease_key) == @lease_nonce
  ...
  def ingest(data)
    Toybaco::Connections::GmailIngest.new(@channel, data, api: integration.api, access_token: @token).call
  end
