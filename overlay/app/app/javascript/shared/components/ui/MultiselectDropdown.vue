...
const onEscape = event => {
  if (!showSearchDropdown.value || event.isComposing || event.keyCode === 229) {
    return;
  }
  // Document shortcuts blur inputs on keydown, so a keyup handler can miss
  // Escape. Close this dropdown before those shortcuts or its parent run.
  event.preventDefault();
  event.stopPropagation();
  onCloseDropdown();
  nextTick(() => dropdownTrigger.value?.$el?.focus({ preventScroll: true }));
};
...
<template>
  <OnClickOutside @trigger="onCloseDropdown">
    <div class="relative w-full mb-2" @keydown.esc="onEscape">
      <Button
        ref="dropdownTrigger"
        slate
        outline
