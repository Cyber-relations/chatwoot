...
const { t, tm } = useI18n();
// The dependency's default footer is English-only. Its public footer slot
// keeps the same keyboard help while inheriting the active application locale.
const { t: footerT } = useI18n({
  useScope: 'local',
  messages: {
    ja: {
      SELECT: '選択',
...
<template>
  <ninja-keys
    ...
    @closed="onClosed"
  >
    <div slot="footer" class="toybaco-command-footer">
      <span><kbd>↵</kbd>{{ footerT('SELECT') }}</span>
      <span><kbd>↑ ↓</kbd>{{ footerT('NAVIGATE') }}</span>
      <span><kbd>Esc</kbd>{{ footerT('CLOSE') }}</span>
      <span><kbd>⌫</kbd>{{ footerT('PARENT') }}</span>
    </div>
  </ninja-keys>
...
<style lang="scss">
.toybaco-command-footer {
  display: flex;
  flex-wrap: wrap;
  gap: 8px 16px;
  padding: 12px 16px;
  color: var(--ninja-secondary-text-color, #667788);
  background: var(--ninja-footer-background, rgba(242, 242, 242, 0.4));
  border-top: var(--ninja-separate-border, 1px solid #e5e7eb);
  font-size: 12px;
  line-height: 1.5;
