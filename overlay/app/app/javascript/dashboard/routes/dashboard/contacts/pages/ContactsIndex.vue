...
const updatePageParam = (page, search = '') => {
  // A completed background contact fetch must not dismiss the posting or AI
  // workspace. These overlays own the current hash outside Vue's route cache.
  const hash = window.location.hash;
  if (hash === '#/toybaco/assistant' || /^#\/toybaco\/posting(?:\?|$)/.test(hash)) {
    return;
  }
