...
const focusActiveOrFirstItem = async () => {
  const request = props.focusRequest;
  if (!request) return;
  await nextTick();
  const popover = popoverRef.value;
  if (props.focusRequest !== request || !popover?.isConnected) return;
  const target =
    popover.querySelector(
      '[data-toybaco-sidebar-focusable][aria-current="page"]'
    ) || popover.querySelector('[data-toybaco-sidebar-focusable]');
  target?.focus();
};

watch(() => props.focusRequest, focusActiveOrFirstItem, { flush: 'post' });
