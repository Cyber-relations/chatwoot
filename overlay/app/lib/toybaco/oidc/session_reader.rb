...
class Toybaco::Oidc::SessionReader
  ...
  def user
    ...
    # uid(email) は変更・重複し得るため一意キーとは扱わない。同じ uid の候補を
    # すべて bcrypt 照合し、現在の client token が exactly 1 人だけに有効な場合に限る。
    # 0 人だけでなく複数一致も fail closed にし、Postiz の sub には User ID を使う。
    valid_users = User.where(uid: uid).to_a.select do |candidate|
