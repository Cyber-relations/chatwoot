...
require_relative 'gmail_parts'
require_relative '../entitlements'

module Toybaco # rubocop:disable Style/ClassAndModuleChildren
  module Connections
    class GmailIngest
      KEY = 'toybaco_gmail_received'

      def initialize(channel, data, api:, access_token:)
        @channel = channel
        @data = data
        limit = Entitlements.for_account(channel.account)&.dig('limits', 'inbound_attachment_bytes') || GmailParts::MAX_ATTACHMENT_BYTES
        @parts = GmailParts.new(api: api, access_token: access_token, data: data,
      ...
      def record!(message)
        receipt = { 'provider_id' => @data.fetch('id'), 'thread_id' => @data['threadId'],
      ...
      def omission_notice
        ...
        labels = { 'file_too_large' => '25MB超過', 'attachment_count' => '添付数の上限', 'not_available' => '提供元で取得不可' }
