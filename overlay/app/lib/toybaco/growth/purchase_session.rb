    ...
    class PurchaseSession
      ...
      def cancel!
        ...
        if session['status'] == 'open'
          session = @client.expire_checkout_session(session.fetch('id'), idempotency_key: "toybaco-expire:#{saved.fetch('nonce')}")
        end
        record!(session, saved)
      end

      def refresh!
        saved = PurchaseIntent.saved(@account.reload)
      ...
      def retrieve(saved)
        ...
        @client.create_checkout_session(saved.fetch('params'), idempotency_key: "toybaco-purchase:#{saved.fetch('nonce')}")
      end
      ...
      def recover(saved)
        ...
        20.times do
          ...
          matches.concat(page['data'].select { |session| session.dig('metadata', 'toybaco_purchase_nonce') == saved['nonce'] })
      ...
      def record!(session, original)
        ...
        @account.with_lock do
          current = PurchaseIntent.saved(@account)
      ...
      def retain_state?(current, session)
        %w[complete expired].include?(current['state']) || (current['state'] == 'payment_pending' && session['status'] == 'open')
      end
