...
REPLACEMENTS = {
  ...
  'app/javascript/dashboard/components/widgets/conversation/ReplyBox.vue' => [
    ...
    replacement("        this.emailRecipientsExpanded = false;\n        // Only update email fields",
                "        this.emailRecipientsExpanded = false;\n        this.toybacoImportedAiDraft = null;\n        // Only update email fields"),
    replacement("    message() {\n      // Autosave",
                "    message() {\n      if (!this.hasMeaningfulEditorContent) this.toybacoImportedAiDraft = null;\n      // Autosave"),
    replacement("      const conversationId = this.currentChat?.id;\n      if (!draft",
                "      const conversationId = this.currentChat?.id;\n      const accountId = this.$route?.params?.accountId;\n      if (!draft"),
    replacement("      if (this.currentChat?.id !== conversationId || this.toybacoAiDraft?.id !== draft.id ||\n          this.isEditorDisabled",
                "      if (this.currentChat?.id !== conversationId || this.$route?.params?.accountId !== accountId ||\n          this.toybacoAiDraft?.id !== draft.id || this.isEditorDisabled"),
    replacement("      this.addIntoEditor(draft.content);\n    },",
    ...
    replacement("            <p>内部メモに保存されています。返信欄に入れて確認するまで、お客さまには送信されません。</p>\n            <p v-if=\"hasMeaningfulEditorContent || (isPrivate && hasAttachments)\">入力中の内容を残しています。内部メモから必要な部分をコピーして追加してください。</p>",
